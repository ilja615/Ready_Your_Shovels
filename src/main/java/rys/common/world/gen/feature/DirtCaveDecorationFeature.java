...
public class DirtCaveDecorationFeature extends Feature<SphereConfig> {
	
	public DirtCaveDecorationFeature(Function<Dynamic<?>, ? extends SphereConfig> configFactoryIn) {
		...
		for (int x = 0; x < config.radius * 2; x++) {
			for (int z = 0; z < config.radius * 2; z++) {
				for (int y = 0; y < config.radius * 2; y++) {
					...
					if (rand.nextFloat() < config.integrity && pos.distanceSq(pos_1) < config.radius * config.radius && config.state.isValidPosition(worldIn, pos_1) && worldIn.isAirBlock(pos_1)) {
